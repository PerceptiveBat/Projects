...
def buy():
    ...
    if request.method == 'GET':
        ...
        if int(shares) < 0 or (shares % 2) != 0:
            return apology('Shares Value Not Positive Integer')
        ...
        if user_cash < transaction_value:
            return apology('Not Enough Money, Peasant!')
...
def add_cash():
    ...
    if request.method == 'GET':
        ...
        user_id = session['user_id']
        cash = int(request.form.get("add_cash"))

        user_cash_db = db.execute('SELECT cash FROM users WHERE id = ?', user_id)
        user_cash = user_cash_db[0]['cash']

        new_cash = user_cash + cash
...
def register():
    ...
    if request.method == 'GET':
        ...
        if not password:
            return apology('No Password Given')
        if not confirmation:
            return apology('No Password Confirmation Given')
        if password != confirmation:
            return apology('Must Input Same Password In Both Fields')
...
def sell():
    ...
    if request.method == 'GET':
        ...
        if shares < 0:
            return apology('Shares Value Not Positive')
        ...
        user_shares_value = user_shares[0]['shares']


        if  shares> user_shares_value:
